...
export default function MapView({ destination, days = [], height = 380 }) {
  ...
  if (error) return <Card className="p-6 text-center text-gray-500">{error}</Card>
  ...
  return (
    <Card className="overflow-hidden">
      <div className="border-b border-gray-100 p-4">
        ...
        <p className="text-xs text-gray-400">Destination marker and nearby itinerary context</p>
      </div>
      <div style={{ height }}>
      ...
      {places.length > 0 && (
        <div className="border-t border-gray-100 p-4">
          <p className="mb-2 text-xs font-medium text-gray-500">Key Places</p>
          <div className="flex flex-wrap gap-2">
            {places.map(place => (
              <span key={place} className="rounded-full bg-blue-50 px-2 py-1 text-xs text-primary">
                {place}
              </span>
            ))}
          </div>
